...
class RegisterViewController: UIViewController, UITextFieldDelegate {
    ...
    // make new user while checking:
    // the emaile is valid and is already regestered in the database and the password is more than 6 characters and the fields are not empty
    @objc func signUp() {
        guard
...
extension RegisterViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    // image picker to let the user choose a profile pic from gallary or camira
...
//struct of user with the user info
struct ChatAppUser {
